 ...
 * PROTEGIDO POR LEI DE DIREITOS AUTORAIS (LEI 9.610/98)
 */
"use strict";

(function() {
    // Marca d'água digital para rastreio judicial
    const _LICENSE_KEY = "MONEYZEN-CS-2026-PRO-SECURITY-BY-CSILVA";
    ...
    window.adicionar = function(tipo) {
        ...
        if (!nome || isNaN(valor) || valor <= 0) return alert("Insira dados válidos.");
    ...
    function atualizarInterface() {
        ...
        transacoes.forEach(t => {
            ...
            if (t.tipo === 'renda') { r += t.valor; hR += item; } 
            else { d += t.valor; hD += item; }
        });
    ...
    function renderGrafico(r, d) {
        ...
        meuGrafico = new Chart(ctx, {
            ...
            options: { 
                plugins: { 
                    legend: { labels: { color: document.body.classList.contains('dark-mode') ? '#ffffff' : '#333333' } } 
                } 
            }
        });
    }
    ...
    window.zerarDados = function() {
        if (confirm("⚠️ Deseja apagar todo o seu histórico exclusivo MoneyZen CS?")) {
    ...
    window.exportarPDF = async function() {
        ...
        doc.text(`ID DE AUTENTICIDADE: ${_LICENSE_KEY}`, 105, 26, { align: "center" });
        ...
        doc.setFontSize(8).text("© 2026 MoneyZen CS. Propriedade Intelectual de C. Silva. Proibida Cópia.", 105, 285, { align: "center" });
        doc.save(`MoneyZen_Relatorio_Protegido.pdf`);
    };
})();
